...
int genkey_main(int argc, const char *argv[])
{
	...
	if (argc != 2) {
		fprintf(stderr, "Usage: %s %s Unsigned int HEX number 0x00000000 as default keyID\n", PROG_NAME, argv[0]);
		return 1;
	}

	keyID = strtol(argv[1], NULL, 0); 

	if (keyID == 0) {
		keyID = OBJID_device_key;
	}
	...
	memzero_explicit(key, WG_KEY_LEN);
	memcpy (key, &keyID, 4);
